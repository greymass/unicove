...
<Box>
	...
	{:else if !metaMaskState.installedSnap}
		<Card class="mb-4">
			<h2 class="mb-2 text-xl font-semibold">Step 2: Install Vaulta Wallet</h2>
			<p class="mb-2">Install the Vaulta Wallet for MetaMask:</p>
			<Button onclick={() => requestSnap(metaMaskState)}>Install Vaulta Wallet</Button>
		</Card>
	{:else}
		<Card class="mb-4">
	...
	<Card class="mt-8 gap-2">
		...
		<p>
			This signup method allows you to use your MetaMask wallet to interact with {data.network.chain
				.name}. The Vaulta Wallet Snap extends MetaMask's functionality, enabling it to sign {data
