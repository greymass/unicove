		...
		<Sidebar.Content tag="main" threshold="80%">
			...
			<Stack class="space-y-12">
				<Stack id="typography">
					...
					<p>
						Styles are applied separately from their tag since there will be situations where we
						want to mix and match styles and tags. We want to adhere to valid and semantic html as
						much as possible, and having a utility class to apply heading styles helps us with that
						goal.
					</p>
					<p>
...
"key2": "someReallyLongValueThatWeCanEitherWrapToTheNextLineOrPresentAScrollbarIfItDoesntFitInThisContainer",
				...
				<Stack id="layout" class="space-y-8">
					...
					<Stack>
						...
						<Center>
							<Card class="box-content max-w-prose">
								<Stack>
									<p>
										Switches the context to content-box and centers the element. No padding by
										default so add a px-4 or similar to avoid being flush to the edge, depending on
										context.
									</p>
									<p>
										Includes a default maximum width which can be overridden with the max-w-
										tailwind class.
									</p>
									<p class="text-center">
					...
					<Stack>
						...
						<p>
							The Sidebar is used for two horizontally adjecent elements: one smaller and fixed
							width, the other filling the remaining space. In a narrow context the elements will
							stack vertically.
						</p>
					...
					<Stack>
						<h3 class="h3">Grid</h3>
						<p>
							The Grid component is used to create a grid layout with a default gap between
							children.
						</p>
						<p>
							The grid is different from the Cluster component in that all children are the same
							size and will align to columns.
						</p>
