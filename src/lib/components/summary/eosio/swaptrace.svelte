<script lang="ts">
	...
	import * as VaultaContract from '$lib/wharf/contracts/core.vaulta';
	...
	interface Props extends Omit<ActionSummaryProps, 'data'> {
		data: VaultaContract.Types.swaptrace;
	}
...
<AssetElement class="col-start-2 col-end-4" value={Asset.from(data.quantity)} variant="full" />
<AccountElement class="col-start-4 col-end-12" name={Name.from(data.account)} />
