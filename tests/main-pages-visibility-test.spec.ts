...
test('test', async ({ page }) => {
  ...
  await page.goto('https://dev.unicove.com/en/jungle4/debug/account');
  await page.getByRole('button', { name: 'Login' }).click();
  await expect(page.getByLabel('account-switcher-label').locator('div')).toContainText('wharfkit1133');

  // Go to the main page.
  await page.getByRole('link', { name: 'My Account' }).click();
  await expect(page.getByRole('heading', { name: 'wharfkit1133 Copy' }).locator('span')).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Jungle 4 (Testnet) Network' }).locator('span')).toBeVisible();
